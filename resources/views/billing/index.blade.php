            ...
            <section class="rounded-xl border-2 border-indigo-500 bg-white p-7 shadow-sm dark:bg-gray-800">
                ...
                @if(! $activeSubscription)
                    <div class="mt-6 grid gap-3">
                        @if($stripeConfigured)<form method="POST" action="{{ route('billing.stripe') }}">@csrf<button class="w-full rounded-lg bg-gray-950 px-4 py-2.5 text-sm font-semibold text-white">Upgrade with Stripe test mode</button></form>@else<button disabled class="w-full cursor-not-allowed rounded-lg bg-gray-950 px-4 py-2.5 text-sm font-semibold text-white opacity-40">Stripe test checkout unavailable</button>@endif
                        @if($paypalConfigured)<form method="POST" action="{{ route('billing.paypal') }}">@csrf<button class="w-full rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white">Upgrade with PayPal sandbox</button></form>@endif
                        @if($razorpayConfigured)<form method="POST" action="{{ route('billing.razorpay') }}">@csrf<button class="w-full rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white">Upgrade with Razorpay test mode</button></form>@endif
                    </div>
                    @if(! $stripeConfigured)<p class="mt-3 text-xs text-gray-500">Stripe test checkout needs its test secret key and recurring Price ID in the deployment environment.</p>@endif
                @else
