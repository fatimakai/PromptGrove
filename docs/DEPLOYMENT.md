...
## 3. Configure live sandbox integrations
...
| Razorpay test | `RAZORPAY_KEY_ID` (`rzp_test_...` only), `RAZORPAY_KEY_SECRET`, `RAZORPAY_PLAN_ID`, `RAZORPAY_WEBHOOK_SECRET` | Create a recurring USD 9/month **test** plan; webhook `https://YOUR-SERVICE.onrender.com/api/webhooks/razorpay`. Subscribe to the subscription events listed in the README. |
| PayPal sandbox | `PAYPAL_CLIENT_ID`, `PAYPAL_CLIENT_SECRET`, `PAYPAL_PLAN_ID`, `PAYPAL_WEBHOOK_ID` | Create a recurring USD 9/month **sandbox** plan; webhook `https://YOUR-SERVICE.onrender.com/api/webhooks/paypal`. Subscribe to the billing subscription events listed in the README. |
...
5. Exercise each sandbox checkout, signed callback and webhook, Pro entitlement, and cancellation. Never enter real payment details.
